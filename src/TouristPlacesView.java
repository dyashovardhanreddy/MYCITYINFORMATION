...
import java.awt.*;
import javax.swing.*;
public class TouristPlacesView extends JFrame{
	...
	public TouristPlacesView()
	{
		...
		{  
			...
			ResultSet rs=stmt.executeQuery("select place_name from tourist_places");  
			while(rs.next())  
			{
		...
		cmbplaces=new JComboBox(placeList.toArray());
		...
		this.setTitle("Tourist Places: View");
		...
		cmbplaces.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				{  
					...
					ResultSet rs=stmt.executeQuery("select * from tourist_places where place_name='"+cmbplaces.getSelectedItem()+"'");  
					while(rs.next())  
					{
						txtplacename.setText(rs.getString(1));
						txtlandmark.setText(rs.getString(2));
						txtrating.setText(rs.getString(3));
					}
		...
		btnupdate.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try{  
					...
					Statement stmt=con.createStatement();  
					if(txtlandmark.getText().compareTo("")==0 || txtplacename.getText().compareTo("")==0 || txtrating.getText().compareTo("")==0)
							JOptionPane.showMessageDialog(null,"Enter all the Fields");
					else
					{
						String query="update tourist_places set place_name='"+txtplacename.getText()+"',landmark='"+txtlandmark.getText()+"',rating="+Float.parseFloat(txtrating.getText())+" where place_name='"+cmbplaces.getSelectedItem()+"'";
						int x=stmt.executeUpdate(query);
						txtmessage.setText(x+" rows updated ");
						con.commit();
						placeList=new ArrayList();
						ResultSet rs=stmt.executeQuery("select place_name from tourist_places");  
						while(rs.next())  
						...
						int count=cmbplaces.getItemCount();
						for(int i=0;i<count;i++)
							cmbplaces.removeItemAt(0);
						for(int i=0;i<placeList.size();i++)
							cmbplaces.addItem(placeList.get(i));
