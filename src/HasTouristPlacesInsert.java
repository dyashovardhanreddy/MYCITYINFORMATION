...
import javax.swing.*;
public class HasTouristPlacesInsert extends JFrame{
	private JPanel panel2;
	private JButton btnHome;
	private JPanel panel;
	private JLabel lblcityname;
	private JLabel lblplacename;
	private JComboBox cmbcityList;
	private JComboBox cmbplaceList;
	...
	public HasTouristPlacesInsert()
	{
		try{  
			...
			ResultSet rs=stmt.executeQuery("select distinct cityname from mycity");  
			while(rs.next())  
				cityList.add(rs.getString(1));
			rs=stmt.executeQuery("(select place_name from tourist_places) minus (select place_name from has_tourist_places)");  
			while(rs.next())
		...
		cmbcityList=new JComboBox(cityList.toArray());
		cmbplaceList=new JComboBox(placeList.toArray());
		panel2=new JPanel();
		...
		this.setTitle("Has Tourist Places : Insert");
		this.add(panel);
		...
		panel.add(cmbcityList);
		panel.add(lblplacename);
		panel.add(cmbplaceList);
		panel.add(btninsert);
		...
		btninsert.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				try{  
					if(cmbcityList.getSelectedIndex()==0 )
					{
						JOptionPane.showMessageDialog(null, "Select City");
					}
					else if(cmbplaceList.getSelectedIndex()==0)
					{
						JOptionPane.showMessageDialog(null, "Select Place");
					}
					else
					{
						Class.forName("oracle.jdbc.driver.OracleDriver");   
						...
						int rows=stmt.executeUpdate("insert into has_tourist_places values('"+cmbcityList.getSelectedItem()+"','"+cmbplaceList.getSelectedItem()+"')");  
						txtmessage.setText("Inserted "+rows+" row"); 
						placeList=new ArrayList();
						placeList.add("-");
						con.commit();
						ResultSet rs=stmt.executeQuery("(select place_name from tourist_places) minus (select place_name from has_tourist_places)");  
						while(rs.next())
							placeList.add(rs.getString(1));
						int count=cmbplaceList.getItemCount();
						for(int i=0;i<count;i++)
							cmbplaceList.removeItemAt(0);
						for(int i=0;i<placeList.size();i++)
							cmbplaceList.addItem(placeList.get(i));
						con.close();  
					} 
					}catch(Exception e){ System.out.println(e);}  
			}
