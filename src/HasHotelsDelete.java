...
import javax.swing.*;
public class HasHotelsDelete extends JFrame{
	...
	public HasHotelsDelete()
	{
		try{  
			...
			hotelList.add("-");
			ResultSet rs=stmt.executeQuery("select distinct cityname from has_hotels");  
			while(rs.next())  
		...
		cmbhotel=new JComboBox(hotelList.toArray());
		...
		this.setTitle("Has Hotels : Delete");
		...
		cmbcityname.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try{  
					...
					hotelList=new ArrayList();   
					hotelList.add("-");
					ResultSet rs=stmt.executeQuery("select hotel_name from has_hotels where cityname='"+cmbcityname.getSelectedItem()+"'");  
					while(rs.next())  
						hotelList.add(rs.getString(1));
					int count=cmbhotel.getItemCount();
					for(int i=0;i<count;i++)
						cmbhotel.removeItemAt(0);
					for(int i=0;i<hotelList.size();i++)
						cmbhotel.addItem(hotelList.get(i)+"");
		...
		cmbhotel.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				txtplacename.setText(cmbhotel.getSelectedItem()+"");
			}
		});
		btndelete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try{  
					...
					int rows=stmt.executeUpdate("delete from has_hotels where cityname='"+txtcityname.getText()+"' and hotel_name='"+txtplacename.getText()+"'");  
					...
					hotelList=new ArrayList();
					rs=stmt.executeQuery("select hotel_name from has_hotels where cityname='"+cmbcityname.getSelectedItem()+"'"); 
					while(rs.next())  
						hotelList.add(rs.getString(1));
