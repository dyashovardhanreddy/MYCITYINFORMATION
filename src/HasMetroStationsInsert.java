...
import javax.swing.*;
public class HasMetroStationsInsert extends JFrame{
	...
	private JComboBox cmbmetrostation;
	...
	public HasMetroStationsInsert()
	{
		try{  
			...
			rs=stmt.executeQuery("(select station_name from metrostations) minus (select station_name from has_metrostations)");  
			while(rs.next())
		...
		this.setTitle("Has MetroStation : Insert");
		this.add(panel);
		...
		btninsert.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				try{  
					if(cmbcityname.getSelectedIndex()==0 )
					{
						JOptionPane.showMessageDialog(null, "Select City");
					}
					else if(cmbmetrostation.getSelectedIndex()==0)
					{
					...
					else
					{
						...
						int rows=stmt.executeUpdate("insert into has_metrostations values('"+cmbcityname.getSelectedItem()+"','"+cmbmetrostation.getSelectedItem()+"')");  
						txtmessage.setText("Inserted "+rows+" row"); 
						metrostationList=new ArrayList();
						metrostationList.add("-");
						con.commit();
						ResultSet rs=stmt.executeQuery("(select station_name from metrostations) minus (select station_name from has_metrostations)");  
						while(rs.next())
							metrostationList.add(rs.getString(1));
