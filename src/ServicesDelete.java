...
public class ServicesDelete extends JFrame{
	...
	public ServicesDelete()
	{
		...
		try{  
			...
			ResultSet rs=stmt.executeQuery("select distinct cityname from services");  
			while(rs.next())
			{
		...
		{
			txt_message.setText(e+"");
		}  
		panel2=new JPanel();
		btnHome=new JButton("Home");
		panel2.setBackground(Color.DARK_GRAY);
		panel=new JPanel(new FlowLayout());
		lbl_cityname=new JLabel("CityName:");
		lbl_service=new JLabel("Service:");
		cmbCityname=new JComboBox(citynames.toArray());
		cmbServices=new JComboBox();
		txt_message=new JTextArea(10,40);
		...
		panel.add(lbl_cityname);
		panel.add(lbl_service);
		...
		cmbCityname.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try{  
					...
					Statement stmt=con.createStatement();  
					servicesList=new ArrayList();   
					ResultSet rs=stmt.executeQuery("select services from services where cityname='"+cmbCityname.getSelectedItem()+"'");  
					while(rs.next())  
						servicesList.add(rs.getString(1)+"");
					int itemCount = cmbServices.getItemCount();
				    for(int i=0;i<itemCount;i++){
				        cmbServices.removeItemAt(0);
				     }
					for(int i=0;i<servicesList.size();i++)
						cmbServices.addItem(servicesList.get(i)+"");
					con.close();  
					  
					}
				catch(Exception ex)
				{ 
					txt_message.setText(ex+"");
				}  
			}
		});
		btn_delete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try{  
					...
					Statement stmt=con.createStatement();  
					String cityname=cmbCityname.getSelectedItem()+"";
					String service=cmbServices.getSelectedItem()+"";
					int x=stmt.executeUpdate("delete  from services where cityname='"+cityname+"' and services='"+service+"'");  
					txt_message.setText("Delete "+x+" rows successfuly");  
					ResultSet rs=stmt.executeQuery("select distinct cityname from services");
					citynames=new ArrayList();
					while(rs.next())
					{
					...
					{
						cmbCityname.addItem(citynames.get(i)+"");
					}
					servicesList=new ArrayList();   
					rs=stmt.executeQuery("select services from services where cityname='"+cmbCityname.getSelectedItem()+"'");  
					while(rs.next())  
						servicesList.add(rs.getString(1)+"");
					itemCount = cmbServices.getItemCount();
				    for(int i=0;i<itemCount;i++){
				        cmbServices.removeItemAt(0);
				     }
					for(int i=0;i<servicesList.size();i++)
