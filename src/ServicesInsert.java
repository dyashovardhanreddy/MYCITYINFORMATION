...
import javax.swing.*;
public class ServicesInsert extends JFrame{
	...
	private String ServicesList[]= {"Metrorailways","hotels","touristplaces"};
	public ServicesInsert()
	{
		try{  
			...
			citylist.add("-");
			ResultSet rs=stmt.executeQuery("((select cityname from mycity )minus(select distinct cityname from services)) union (select cityname from services group by cityname having count(cityname)<3 )");  
			while(rs.next())  
		...
		cmbServices=new JComboBox<String>(ServicesList);
		...
		btn_insert.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try{  
					...
					{
						String newCityname=cmbcityname.getSelectedItem()+"";
						Statement stmt=con.createStatement();
						String query="insert into services values('"+newCityname+"','"+cmbServices.getSelectedItem()+"')";
