...
import javax.swing.*;
import java.awt.*;
public class MyCityTableView extends JFrame{
	...
	public MyCityTableView()
	{
		...
		this.setTitle("MyCity Table View");
		...
		btn_update.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try{  
					...
					{
						JOptionPane.showMessageDialog(null, "Select cityname to be modified and add changes");
					}
					else
					{
						String modifiedCityname=txt_cityname.getText();
						Statement stmt=con.createStatement();
						String query="update mycity set cityname='"+modifiedCityname+"' where cityname='"+cmbCityName.getSelectedItem()+"'";
						int x=stmt.executeUpdate(query);
						con.commit();
						messageBox.setText("\nUpdated " + x + " rows successfully");
						ResultSet rs=stmt.executeQuery("select cityname from mycity");
						cityList=new ArrayList<String>();
						while(rs.next())     
							cityList.add(rs.getString(1));
